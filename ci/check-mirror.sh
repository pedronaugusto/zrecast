...
# failure can be reproduced on the machine that caused it. In a sibling
# package nothing held that claim, and it was false: the workflow built a
# configuration the local roster did not, so an option declared in `build.zig`
# could be broken for as long as it took someone to open a pull request.
#
...
# The roster invokes the compiler as `$ZIG build` (overridable toolchain), the
# workflow as `zig build`; both spellings name a build step.
combinations() {
  grep -hE '(zig|\$ZIG) build' "$1" | tr -d '\r\042\047' | awk '
    {
      n = 0
      for (i = 1; i <= NF; i++)
